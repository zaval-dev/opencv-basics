...
def batman_mask(img):
    ...
    for (x, y, w, h) in face_rects:
        # Ajustar el tamaño y la posición del área para la máscara
        ...
        # Extraer la región de interés (ROI) de la imagen donde se colocará la máscara
        img_roi = img[y:y+h, x:x+w]

        # Redimensionar la máscara al tamaño del rostro detectado
        face_mask_small = cv2.resize(face_mask, (w, h), interpolation=cv2.INTER_AREA)

        # Convertir la máscara a escala de grises y crear una máscara binaria
        ...
        # Aplicar la máscara sobre la región de interés y la máscara de Batman
        masked_face = cv2.bitwise_and(face_mask_small, face_mask_small, mask=mask)
        masked_img = cv2.bitwise_and(img_roi, img_roi, mask=mask_inv)

        # Combinar las dos imágenes: la región original y la máscara aplicada
...
def nose_circle_filter(img):
    ...
    # Detectar la nariz en la imagen en escala de grises
    nose_rects = nose_cascade.detectMultiScale(gray, scaleFactor=1.3, minNeighbors=5)

    if len(nose_rects) == 0:
        print("No se detectó ninguna nariz.")
        return img  # Si no se detecta nariz, devolver la imagen original

    for (x, y, w, h) in nose_rects:
        # Ajustar el tamaño de la máscara del círculo al tamaño de la nariz detectada
        mask_width = int(2 * w)  # Ajustar el tamaño del círculo según el ancho de la nariz detectada
        mask_height = int(1.5 * h)  # Ajustar el tamaño del círculo según la altura de la nariz detectada
        resized_circle_mask = cv2.resize(circle_mask, (mask_width, mask_height), interpolation=cv2.INTER_AREA)

        # Comprobar si la imagen tiene canal alfa
        if resized_circle_mask.shape[2] == 4:  # Si tiene canal alfa
            bgr_mask = resized_circle_mask[:, :, :3]  # Canales BGR
            alpha_mask = resized_circle_mask[:, :, 3]  # Canal alfa
        else:
            bgr_mask = resized_circle_mask
            alpha_mask = cv2.cvtColor(bgr_mask, cv2.COLOR_BGR2GRAY)  # Convertir a escala de grises si no tiene alfa

        # Crear una máscara binaria a partir del canal alfa
        ...
        # Verificar que las dimensiones de ROI y la máscara coincidan
        ...
        # Aplicar la máscara sobre la región de interés
        ...
        # Combinar la región de interés original y la máscara aplicada
